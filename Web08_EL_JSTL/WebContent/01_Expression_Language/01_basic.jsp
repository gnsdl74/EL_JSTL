...
<body>
	<%-- 
	 ...
	 - param : 요청 파라미터의 <파라미터이름, 값> 매핑을 저장한 Map 객체, 타입은 String(request.getParameter("파라미터 이름")의 결과와 동일)
	 - paramValues : 요청 파라미터의 <파라미터이름, 값> 매핑을 저장한 Map 객체, 타입은 String[] (request.getParameterValues("파라미터 이름")의 결과와 동일)
	 - cookie : <쿠키이름, 쿠키값> 매핑을 저장한 Map 객체 (request.getCookies()로 리턴된 Cookie 배열형태)
	 - initParam : 초기화된 파라미터의 <파라미터이름, 값> 매핑을 저장한 Map 객체
	   (application.getInitParameter("파라미터 이름")의 결과와 동일)
	...
	<%
		...
		int number3 = (Integer) session.getAttribute("num3");
		int number4 = (Integer) application.getAttribute("num4");
	...
	<p>request num2 : ${num2 }</p> <%--request.getAttribute("num1") --%>
	<p>session num3 : ${num3 }</p> <%--session.getAttribute("num1") --%>
	<p>application num4 : ${num4 }</p> <%--application.getAttribute("num1") --%>
